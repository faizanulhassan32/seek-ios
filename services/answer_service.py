...
class AnswerService:
        ...
        try:
            ...
            if not is_valid:
                ...
                if candidate_desc:
                    ...
                    # Return original answer if no fallback possible, or maybe a generic apology?
                    # For now, return original to avoid breaking flow, or empty string?
                    # Let's return original but log it.
    ...
    def generate_fallback_summary(self, query: str, candidate_context: str) -> str:
        ...
        try:
            input_prompt = f"""
            Summarize the following known data into a 2-paragraph profile for {query}. 
            Do not add external info, just professionalize this.
    ...
    def extract_osint_data(self, text_content: str) -> Dict:
        ...
        """
        try:
            # Truncate text to avoid token limits (keep first 4000 chars - usually has the header info)
            ...
            # We assume the model follows instruction to return JSON if we format it right or just parse the text.
            # Actually, `responses` API (if this represents the new feature) might return structured output if requested?
            # The current usage seems to just return .output_text. 
            # Let's trust it returns a valid JSON string or we can ask for JSON in prompt.
            
            # Since we can't depend on strict JSON mode in this client wrapper (maybe?), 
            # let's try to parse the output text.
